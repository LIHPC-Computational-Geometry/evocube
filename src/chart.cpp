...
std::vector<Eigen::RowVector3d> perBorderAxis(const std::vector<std::pair<int, int>>& patches_per_border,
    ...
    for (int i=0; i<patches_per_border.size(); i++){
        ...
        if (label1/2 != label2/2){
            ...
            // boundary between two opposite labels, axis ill defined
            // arbitrary value, should not be used
            axis(0) = -1.0;
            axis(1) = -1.0;
            axis(2) = -1.0;
        }
        per_border_axis.push_back(axis);
    }
    return per_border_axis;
}
    ...
    for (int i=0; i<TT.rows(); i++){
        for (int neigh=0; neigh<3; neigh++){
            ...
            if (charts(i) != charts(TT(i, neigh))){ // this edge is on a border
                ...
                border_triangles.push_back(i);
                border_triangles.push_back(TT(i, neigh));
            }
        }
    }
